...
with tf.Graph().as_default():
    ...
    with tf.name_scope('hidden'):
        weights = tf.Variable(tf.truncated_normal([NUM_INPUT_UNITS, HIDDEN_UNITS], 
    ...
    with tf.name_scope('out_layer'):
        weights = tf.Variable(
            tf.truncated_normal([HIDDEN_UNITS, NUM_OUTPUT_UNITS],
                                stddev=1.0 / HIDDEN_UNITS),
            name='weights')
    ...
    with tf.name_scope('loss_layer'):
        ...
        loss = tf.reduce_sum(tf.div(tmp + 1e-9, TOTAL_PIXELS), name='loss')
